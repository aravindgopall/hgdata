...
listBookmarks email password smsToken = do
  now <- getCurrentTime
  withManager defaultManagerSettings $ \manager -> do
    requestGet1 <-
      parseUrl $
      "https://accounts.google.com/Login?continue=" ++
      listingUrl ++ "&hl=en&service=bookmarks&authuser=0"
    responseGet1 <- httpLbs requestGet1 manager
    let encode = LBS8.unpack . fromString
        ...
        requestPost1 =
          (accountsPostRequest "/ServiceLoginAuth")
            { requestBody =
                ...
                "&service=bookmarks" ++
                "&dsh=" ++
                extractValue "dsh" bodyGet1 ++
                "&GALX=" ++
                extractValue "GALX" bodyGet1 ++
                "&bgresponse=js_disabled" ++
                "&Email=" ++
                encode email ++
                "&Passwd=" ++ encode password ++ "&PersistentCookie=yes"
            , cookieJar = Just cookieJarGet1
            , redirectCount = 0
            , checkResponse = \_ _ -> return ()
            }
    responsePost1 <- httpLbs requestPost1 manager
    let cookieJarPost1 = responseCookieJar responsePost1
        requestPost2 =
          (accountsPostRequest "/SmsAuth")
            { queryString =
                BS8.pack $ "?continue=" ++ listingUrl ++ "&service=bookmarks"
            , requestBody =
                RequestBodyBS $
                BS8.pack $
                "continue=" ++
                ...
                "&service=bookmarks" ++
                "&exp=smsauthnojs" ++
                "&smsUserPin=" ++ encode smsToken ++ "&PersistentCookie=yes"
            , cookieJar = Just cookieJarPost1
            , redirectCount = 0
            , checkResponse = \_ _ -> return ()
            }
    responsePost2 <- httpLbs requestPost2 manager
    let bodyPost2 = responseXml responsePost2
        ...
        requestPost3 =
          (accountsPostRequest "/ServiceLoginAuth")
            ...
            , requestBody =
                ...
                listingUrl ++
                "&smsToken=" ++
                extractValue "smsToken" bodyPost2 ++
                "&GALX=" ++
                extractValue "GALX" bodyGet1 ++ "&bgresponse=js_disabled"
...
listingUrl :: String
listingUrl =
  "https%3A%2F%2Fwww.google.com%2Fbookmarks%2F%3Foutput%3Dxml%26num%3D100000"

extractValue :: String -> Element -> String
extractValue value root =
  fromJust $ do
    let inputElement = blank_name {qName = "input"}
        nameAttribute = blank_name {qName = "name"}
        valueAttribute = blank_name {qName = "value"}
        filter element =
          elName element == inputElement &&
          findAttr nameAttribute element == Just value
